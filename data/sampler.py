...
class LenClusteringSampler(Sampler):
    def __init__(
        self,
        dataset: MyDataset,
    ...
    def get_local_indices(self) -> List[int]:
        ...
        for i, meta in enumerate(self.dataset.meta_collection):
            ...
            assert len(indices) == len(meta["item_info_list"])
            indices_and_len = [
                [idx, self.dataset.item_processor.len_from_info(info)]
                for idx, info in zip(indices, meta["item_info_list"])
            ]
        ...
        if dist.get_rank() == 0:
            print(f"part1 end, use {time.time() - start_time:.2f}s", flush=True)
            start_time = time.time()
            print("part2 start", flush=True)
        ...
        if dist.get_rank() == 0:
            print(f"part2 end, use {time.time() - start_time:.2f}s", flush=True)
            start_time = time.time()
            print("part3 start", flush=True)

        if self.shuffle:

            if dist.get_rank() == 0:
                print(f"part3 end, use {time.time() - start_time:.2f}s", flush=True)
                start_time = time.time()
                print("part4 start", flush=True)

            for group_name, indices_and_len in group_indices_and_len.items():
                rng.shuffle(indices_and_len)

            if dist.get_rank() == 0:
                print(f"part4 end, use {time.time() - start_time:.2f}s", flush=True)
                start_time = time.time()
                print("part5 start", flush=True)
            ...
            if self.length_clustering:
                ...
                if dist.get_rank() == 0:
                    print(f"part5 end, use {time.time() - start_time:.2f}s", flush=True)
                    start_time = time.time()
                    print("part6 start", flush=True)
                ...
                if dist.get_rank() == 0:
                    print(f"part6 end, use {time.time() - start_time:.2f}s", flush=True)
                    start_time = time.time()
                    print("part7 start", flush=True)

                # option1: shuffle among neighboring items
                for group_name, indices in group_indices.items():
                    result = []
                    for pos in range(0, len(indices), global_batch_size * 100):
                        sublist = indices[pos : pos + global_batch_size * 100]
                        rng.shuffle(sublist)
                ...
                if dist.get_rank() == 0:
                    print(f"part7 end, use {time.time() - start_time:.2f}s", flush=True)
                    start_time = time.time()
                    print("part8 start", flush=True)
            else:
                for group_name, indices_and_len in group_indices_and_len.items():
                    group_indices[group_name] = [_[0] for _ in indices_and_len]
            ...
            if self.allow_mixed_task_among_acc:
                ...
                if dist.get_rank() == 0:
                    print(f"part8 end, use {time.time() - start_time:.2f}s", flush=True)
                    start_time = time.time()
                    print("part9 start", flush=True)
            else:
                global_batched_indices = []
                for group_name, indices in group_indices.items():
            ...
            if dist.get_rank() == 0:
                print(f"part9 end, use {time.time() - start_time:.2f}s", flush=True)
                start_time = time.time()
                print("part10 start", flush=True)

            indices = [_ for batch_indices in global_batched_indices for _ in batch_indices]

            if dist.get_rank() == 0:
                print(f"part10 end, use {time.time() - start_time:.2f}s", flush=True)
                start_time = time.time()
                print("part11 start", flush=True)
        else:
        ...
        # debug only
        # for checking cross-rank uniformity
        print(f"{self.rank}: {indices[::1000][:10]}", flush=True)
    ...
    def __iter__(self) -> Iterator:
        return iter(self.get_local_indices())
